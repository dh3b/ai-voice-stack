Every variable can be set in a `.env` file after copying `.env.example` (the `.env` overrides the baseline). Here is what each setting does.
...
APP_BARGE_IN_ENABLED=true    # saying the wake word mid-reply interrupts it; false plays replies to the end
APP_LOGGING_LEVEL=DEBUG    # DEBUG, INFO, WARNING, ERROR; DEBUG also prints per-turn latency timings
...
# LLM_EXECUTABLE_PATH    # path to llama-server; auto-detected per OS (llama-server[.exe]), uncomment in .env to override
...
LLM_CONTEXT_WINDOW=8192    # context window, set lower if history is disabled
LLM_GPU_LAYERS=0    # layers offloaded to the GPU; 0 is CPU-only, 99 is all (needs a CUDA build)
...
LLM_HISTORY_MAX_TURNS=3    # How many messages (ai-user pairs) back the history should store
LLM_HISTORY_IDLE_TIMEOUT_S=120.0    # After how many idle seconds the earliest message is wiped from memory

TOOLS_ENABLED_MODULES=math_tools,datetime_tools,random_tools,memory_tools    # which tool modules to load (comma-separated)
TOOLS_TOOL_TIMEOUT=15.0    # max seconds per tool call before the model gets a timeout error
TOOLS_MEMORY_SYSTEM_INSTRUCTIONS=    # system instructions appended when memory is on, to guide the agent
TOOLS_MEMORY_DB_PATH=./assets/memory.db    # SQLite file backing persistent memory

OWW_MODEL_PATHS=./models/hey_jarvis_v0.1.onnx    # wake word .onnx model(s), comma-separated
...
STT_LANGUAGE=auto    # set to auto or a specific ISO code
...
STT_RESPONSE_TIMEOUT=5.0    # default listen window, seconds
